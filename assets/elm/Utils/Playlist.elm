...
map : (a -> b) -> Playlist a -> Playlist b
map f (Playlist head current tail) =
    let
        mapper =
            \( i, v ) -> ( i, f v )
    in
    Playlist
        (List.map mapper head)
        (mapper current)
        (List.map mapper tail)


multiMap : (a -> b) -> (a -> b) -> (a -> b) -> Playlist a -> Playlist b
multiMap beforeMapper currentMapper afterMapper =
    let
        mapper f =
            \( i, v ) -> ( i, f v )
    in
    \(Playlist before current after) ->
        Playlist
            (List.map (mapper beforeMapper) before)
            (mapper currentMapper current)
            (List.map (mapper afterMapper) after)


toListWithIndexes : Playlist a -> List ( Int, a )
toListWithIndexes (Playlist head current tail) =
    head ++ [ current ] ++ tail
