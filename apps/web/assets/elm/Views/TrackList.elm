...
import Html exposing (div, span, table, tbody, td, th, thead,  text, tr)
        ...
        (case model of
            RemoteData.Success tracks ->
                [
                    ...
                    , tbody [] (List.map trackView tracks)
                    ]
                ]
...
trackView track =
    let
        displayInfo maybeInfo =
            case maybeInfo of
                Nothing -> span [Attrs.style "color" "red"] [text "missing track title"]
                Just title -> span [] [text title]
    in
